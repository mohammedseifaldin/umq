...
class ChartMultiLineFastor extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    var chart = LineChart(sampleData1);
  ...
  LineTouchData get lineTouchData1 => LineTouchData(
        ...
        touchTooltipData: LineTouchTooltipData(
          getTooltipColor: (_) => colorTouchWindows!,
        ),
      );
  ...
  Widget bottomTitleWidgets(double value, TitleMeta meta) {
    ...
    var styleText = TextStyle(
        fontWeight: FontWeight.bold, fontSize: 12, color: colorTextTitle!);
    var textDynamic = Text(
      textTitleCurrentPosition,
      style: styleText,
    );
  ...
  static int _roundToPlus1(double row_counter) {
    //round
    double fraction =
        row_counter - row_counter.truncate(); // '2.3555' >> "0.3555"
    if (fraction > 0.0) {
  ...
  List<LineChartBarData> getMulitLineDrawingData() {
    ...
    for (int positionLine = 0;
        positionLine < linesDrawingData.length;
        positionLine++) {
      var dataDoubleValue = linesDrawingData[positionLine];
      LineChartBarData singleBar = mapDoubleValueArrayToObjectLineChartBarData(
          positionLine, dataDoubleValue);
  ...
  LineChartBarData mapDoubleValueArrayToObjectLineChartBarData(
      int linePosition, List<double> dataDoubleValue) {
  ...
  double getXAxisByIndexOfDataValue(
      List<double> dataDoubleValue, int indexOfDataYValue) {
