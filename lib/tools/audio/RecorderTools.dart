...
class RecorderTools {
  ...
  static Future<bool> isPermissionDenied(
      BuildContext context, bool isPrintToastFailed) async {
    //check mobile
    if (DeviceTools.isMobile()) {
      bool isDenied = await Permission.microphone.isDenied;
      Log.i("isPermissionDenied() - isDenied: $isDenied");
      bool isGranted = await Permission.microphone.isGranted;
      Log.i("isPermissionDenied() - isGranted: $isGranted");

      var status = await Permission.microphone.request();
      Log.i("isPermissionDenied() - status: $status");
