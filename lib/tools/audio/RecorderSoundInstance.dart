...
class RecorderSoundInstance {
   ...
   Future start(BuildContext context , String currentRecorderKey) async {
    ...
    try {
      ...
      ToolsWait.waitToDo( 100, ()  async {
        ...
        Log.k(tag, "start()  - ok  " + _record.toString());
      });

    } catch (e) {
      Log.k(tag, "start()  - exc: " + e.toString());
  ...
  static  Future _initPath() async  {
    ...
    if( DeviceTools.isAndroid() ) {
      var directory = await getApplicationDocumentsDirectory();
      var directoryPath = directory.path;
      RecorderSoundInstance.pathDefaultFile = directoryPath + "/myRecorder" +  key + ".mp4";
    ...
    if( DeviceTools.isPlatformWeb() ) {
      var directory = await getApplicationDocumentsDirectory();
      var directoryPath = directory.path;
      RecorderSoundInstance.pathDefaultFile = directoryPath + "/myRecorder" +  key + ".webm";
      Log.i( "_initPath() - _mPath: " +  RecorderSoundInstance.pathDefaultFile );
      return;
    }
    ...
    if( DeviceTools.isIOS() ) {
      ...
      Log.i( "_initPath() - _mPath: " +  RecorderSoundInstance.pathDefaultFile.toString() );
      return;
    }
