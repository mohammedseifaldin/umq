...
class SocketContinueDownloadData extends SocketContinueController {
  static Future apiSocketContinue() async {
    ...
    //api
    ChatSocketContinueAPI().getData(
        userLimit: user_limit,
        messageGreaterThanTime:
            SocketContinueController.messageIdQueryGreaterThan,
        indexLoop: SocketContinueController.counterDownloadCompleted,
        countryCode: SocketContinueController.countryCode ?? "",
        callBack:
            _callBack(userPage, SocketContinueController.timePreviousRun));
  }

  static ChatChatSocketContinueAPICallBack _callBack(
      int userPage, int timeFireRequest) {
    return (status, msg, response) {
      //change status to complete
      SocketContinueController.statusProcessId = 2;

      //log
      SocketContinueController.counterDownloadCompleted =
          SocketContinueController.counterDownloadCompleted + 1;
      var counterNew =
          SocketContinueController.counterDownloadCompleted.toString();
      ...
      SocketContinueController.messageIdQueryGreaterThan =
          response.data!.next_socket_search_for_greater_than_id ?? 0;

      Log.i("SocketContinueDownloadData - _callBack() - counterNew: " +
          counterNew +
          " /messageIdQueryGreaterThan: " +
          SocketContinueController.messageIdQueryGreaterThan.toString());
