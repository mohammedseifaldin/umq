...
import 'package:umq/modules/profile/presentation/admin/user/list/m/ExtenstionMapModelToWidget.dart';
import 'package:umq/modules/profile/data/source/UsersListAPI.dart';
...
extension SearchDownloadController on UserListAdminState {
  ...
  Future _listenerDownloadData() async {
    ...
    //listener
   await UsersListAPI().getData(page, search_txt, selectedRole,   (   status ,  msg,   response   ) {
      ...
      if(paginateState != null ){
        paginateState!.setPaginateByRecordTotal(
            response.dataLaravel!.total  ?? 0,
            AdminPanelConstant.perPage,
            response.dataLaravel!.currentPage ?? page
        );
      }
