...
extension ContentUserDetail on UserDetailAdminState {
  ...
  Widget name() {
    var row = RowTemplate.scroll(contextPage!, [tv_name(), tf_name()]);
  ...
  Widget email() {
    var row = RowTemplate.scroll(contextPage!, [tv_email(), tf_email()]);
  ...
  Widget password() {
    var row = RowTemplate.scroll(contextPage!, [tv_password(), tf_password()]);
  ...
  Widget city() {
    var row = RowTemplate.scroll(
        contextPage!, [tv_city(), sp_city_selectedNameColumn()]);
  ...
  Widget sp_city_selectedNameColumn() {
    return ColumnTemplate.t(children: [sp_city(), tv_city_previousSelected()]);
  }

  Widget tv_city_previousSelected() {
    // Log.i( "tv_city_previousSelected() - city_selected_name: " + city_selected_name );
    return TextTemplate.t(city_selected_name,
        levelDS: LevelDS.l4,
  ...
  Widget sp_city() {
    ...
    var spin = SpinnerView(
      ...
      iconSize: 22,
      width_frame: AdminDSDimen.inputFiled_value_width -
          AdminDSDimen.spinnerTriangleWidth,
      height_frame: 35,
  ...
  Widget city_hint() {
    return TextTemplate.t("select city",
        padding: EdgeInsets.all(5), levelDS: LevelDS.l2);
  }
  ...
  Widget mobile() {
    ...
    //fix landscape
    var sizeBox = SizedBox(width: AdminDSDimen.inputFiled_line, child: v);
