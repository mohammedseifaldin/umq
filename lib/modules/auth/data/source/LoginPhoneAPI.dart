...
typedef LoginCallBack = Function(bool status, String msg , ResponseLoginAdmin response);
...
class LoginPhoneAPI {
  ...
  LoginPhoneAPI(this.mobileCountry, this.mobilePhone,  this.pass, this.callBack) {
    startAPI();
  }
  ...
  void startAPI()  {
    ...
    String fullPhone = ToolsPhone.fixEgyptAndSaudiNumber(mobileCountry, mobilePhone);
    Map<String, dynamic > body = Map();

    body[ "username"] = fullPhone;
    body[ "password"] = pass;

    NetworkManagerDio().callBack(url, body: body,  callback: (status, msg, json){
  ...
  void parseJson(Map<String, dynamic> mapJson ) {
    ...
    try {
      ...
      //save object
      UserSingleTone.instance().saveUserLogin(response.data!.user!, response.data!.accessToken!, (){
