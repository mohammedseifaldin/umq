...
extension ExtenstionRowSlider on SliderListAdminState {
  ...
  Future<List<Widget>> listCellForSingleRecord(MSlider m) async {
    ...
    list.add(
        cell_image_single(m.image, 200, height: AdminDSDimen.row_height_xxx));

    //id show
    list.add(cell_id_primary(m.id, AdminDSDimen.header_width_m,
        height: AdminDSDimen.row_height_xxx, () async {
      await click_show_detail(m);
    }));

    //delete
    list.add(cell_button("Trash", AdminDSDimen.header_width_m,
        height: AdminDSDimen.row_height_xxx, () async {
    ...
    list.add(cell_text(time, AdminDSDimen.header_width_l,
        height: AdminDSDimen.row_height_xxx));
