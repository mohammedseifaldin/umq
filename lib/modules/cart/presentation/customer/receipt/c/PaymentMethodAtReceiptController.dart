...
extension PaymentMethodAtReceiptController on CartReceiptState {
  ...
  Future chooseTypePaymentMethod() async {
    ...
    bool isOnline = OrderCurrentSingletone.instance().isPaymentMethodTypeOnline();
    if( isOnline ) {
  ...
  bool isVailedOnlinePaymentMethod()   {
    ...
    bool isOnline = OrderCurrentSingletone.instance().isPaymentMethodTypeOnline();
    if( isOnline ) {
      bool isSuccessOnline = OrderCurrentSingletone.instance().onlinePayment_status;
  ...
  Future  apiCreateOrder() async  {
    ...
    //map data
    OrderRequest request = await MapCurrentOrderToNewOrderRequest.generateFromCurrentOrder(context);
