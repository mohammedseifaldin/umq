...
class CartShipmentPage extends StatefulWidget {
  const CartShipmentPage({super.key});
...
class CartShipmentState extends State<CartShipmentPage> {
  ...
  Widget toolbar() {
    return ToolbarCartView(context, "Shipment", 1, onBackClick: (b) {
  ...
  Widget tvTitle() {
    return TextTemplate.t("Enter your Location",
        gravityLayoutAlign: Alignment.center, levelDS: LevelDS.l2);
  }
  ...
  Widget addressDetailTv() {
    ...
    var tf = TextFieldFastor(
        ...
        onChanged: (s) async {
          addressText = s;
          //save data
          OrderCurrentSingletone.instance().address = addressText;
        }
    ...
    //margin
    return Container(
        child: tf, margin: EdgeInsets.only(top: DSDimen.space_level_2));
  }
  ...
  Widget cityDropdown() {
    ...
    return Container(
        child: cityWidget, margin: EdgeInsets.only(top: DSDimen.space_level_3));
  }
