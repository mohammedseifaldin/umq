...
extension SubscribePackageList on SubscribeChangeNotifier {
  ...
  Future subscribePackageRefreshOrDownloadNextPage(
      {required BuildContext context, required bool isResetPage}) async {
    Log.i(
        "subscribePackageRefreshOrDownloadNextPage() - isResetPage: $isResetPage");
  ...
  Future _getPackagesWithPaginateDataThenNotifyChange(
      BuildContext context) async {
    //wait
    responsePaginateSubscribePackage =
        await SubscribeDataSource.getListPackageWithPaginate(
            _getNextPageNumber());

    _adminPackagesUpdateProgress(false);

    if (responsePaginateSubscribePackage != null &&
        responsePaginateSubscribePackage!.data != null) {
  ...
  Future subscribePackageHidde(BuildContext context, int id) async {
    ...
    var request = RequestSubscribePackageHide(packageId: id, hidden: 1);
    await SubscribeDataSource.hidePackage(request);
    Log.i("subscribePackageHidde() - hidePackage done");

    ///refresh
    await subscribePackageRefreshOrDownloadNextPage(
        context: context, isResetPage: true);
