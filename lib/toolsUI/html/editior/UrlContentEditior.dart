...
extension UrlContentEditior on HtmlEditorState {
  ...
  Future _apiDownloadContentOfUrl() async {
    NetworkManagerHttp(widget.edit_url_content!, type: NetworkTypeHttp.get,
        callback: (status, msg, dataStr) {
      if (status == false) {
        Log.i(
            "UrlContentEditior - _apiDownloadContentOfUrl() - status == false - msg: $msg");
        //progress
        setState(() {
          progressStatus = false;
        });
        ToolsToast.i(
            context, "Failed To Download Old Data Content. Refresh THe Page");
        return;
      }
      ...
      setState(() {
        Log.i(
            "UrlContentEditior - _apiDownloadContentOfUrl() - success download data: $dataStr");
        progressStatus = false;
