...
extension CheckerPaymentStatusComplete on PaypalGenericState {
  ...
  Future checkThisUrlForCompleteStatus(String urlToCheck ) async {
    ...
    if( isTransactionSuccess ) {
      Log.i("checkThisUrlForCompleteStatus() - stop -  isTransactionSuccess equal(true)");
      return;
    }

    //check completed by failed
    bool isFailedUrl = urlToCheck.contains( PayPalConstant.returnUrlFailedTransaction) ;
    if(  isFailedUrl   ) {
    ...
    //check success
    bool isSuccessUrl = urlToCheck.contains( PayPalConstant.returnUrlSuccessTransaction) ;
    if(  isSuccessUrl   ) {
