...
class ButtonPickerUserWidget extends StatefulWidget {
  ...
  ButtonPickerUserWidget(
      {super.key, required this.contextPage, required this.select});
...
class _ButtonPickerUserState extends State<ButtonPickerUserWidget> {
  ...
  Widget build(BuildContext context) {
    var row = RowTemplate.scroll(
        context, [tv_title(), spinnerShapeWithClickOnTouch()]);
  ...
  Widget spinnerShapeWithClickOnTouch() {
    var stack = Stack(children: [
      ...
      Positioned(right: 0, child: spinnerIcon())
    ]);
  ...
  Widget spinnerIcon() {
    return const Icon(Icons.arrow_drop_down, size: 21, color: Colors.black);
  }
  ...
  Future pickerUserDialogOpen() async {
    GoToAdmin.pickerUser(widget.contextPage, null, select: (MUser mUser) {
      Log.i("pickerUserDialogOpen() - mUser $mUser");
