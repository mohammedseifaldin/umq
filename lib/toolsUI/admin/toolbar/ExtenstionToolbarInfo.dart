...
extension ExtenstionToolbarInfo on AdminToolbar {
  Widget barToolbarInfo() {
    ...
    var stackBarInfo = Stack(children: [
      ...
      EmptyView.empty(DeviceTools.getWidth(contextPage),
          AdminDSDimen.toolbarSimple_barInfo),
  ...
  Widget rowControllePanel() {
    return RowTemplate.wrapChildren([title(), tx_controllePanel()]);
  }

  Widget title() {
    return TextTemplate.t(pageTitle,
        margin: EdgeInsets.only(
            left: DSDimen.space_level_1, top: DSDimen.space_level_2),
  ...
  Widget underLine() {
    var widthLine =
        DeviceTools.getWidth(contextPage) - (DSDimen.space_level_1 * 2);
    var line = EmptyView.colored(widthLine, 5,
        DSColor.ds_background_toolbar //same color of controller panel text
        );

    return Container(
      margin: EdgeInsets.only(
          left: DSDimen.space_level_1, right: DSDimen.space_level_1),
      child: line,
    );
  }
}
