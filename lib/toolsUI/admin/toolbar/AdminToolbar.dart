...
class AdminToolbar extends StatelessWidget {
  ...
  Widget getToolbarLayers() {
    return Stack(
      children: [
        //color background
        EmptyView.colored(DeviceTools.getWidth(contextPage), toolbarHeightLayer,
            DSColor.ds_background_all_screen),
        columnToolbarBars()
      ],
    );
  }
  ...
  Widget columnToolbarBars() {
    var col = Column(children: [
      ToolbarSimpleFastor(contextPage, pageTitle,
          buttonLeft: bt_menu(),
  ...
  Widget bt_create() {
    ...
    //button with click
    return ImageView(
      width: size_button_create,
      height: size_button_create,
  ...
  Widget buttonRightRow() {
    return RowTemplate.wrapChildren_gravityCenter([bt_refresh(), bt_create()]);
  }
  ...
  Widget bt_refresh() {
    return ImageViewTemplate(
      context: contextPage,
      widthNeeded:
          size_button_create, //why 0.80 %  i see as my eye it's too big
