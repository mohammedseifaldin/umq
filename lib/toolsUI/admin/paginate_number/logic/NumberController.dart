
import 'package:fastor_app_ui_widget/fastor_app_ui_widget.dart';


import 'package:flutter/material.dart';
import 'package:umq/toolsUI/admin/paginate_number/widget/NumberViewPaginate.dart';
import 'package:umq/toolsUI/admin/paginate_number/PaginateNumberWidget.dart';

extension NumberController on PaginateNumberState {


  //--------------------------------------------------------------------- draw first

  void setupFirstNumber(){
    //remove previous
    listNumberWidget = [];

    //add first number page
    listNumberWidget.add( boxNumber(page: 1, isSelected: true ) );

    //refrseh
    setState(() {

    });
  }

  //--------------------------------------------------------------------- draw list

  void drawListNumberWidget(){
    //remove previous
    listNumberWidget = [];

    //maximue
    int starter = _calculateStarterNeedoShow();
    int counterNeedToShow = _calculateCounterTabButtonNeedToShow();

    //for loop widget
    int position = 1;
    for( int tabIndexNumber = starter ; tabIndexNumber <= widget.pageTotal ; tabIndexNumber++ ){

      //selected
      bool isCurrentPage = tabIndexNumber == widget.currentPage;
      //Log.tabIndexNumber( "drawListNumberWidget() - tabIndexNumber: " + tabIndexNumber.toString() );

      //check arrive maxPage
      if(  tabIndexNumber  >= widget.maxPage!  ) {
        Log.i( "drawListNumberWidget() - maxPage >= tabIndexNumber - maxpage: " +  widget.maxPage.toString()  );
        break;
      }

      //add
      Widget w = boxNumber(page: tabIndexNumber, isSelected: isCurrentPage );
      listNumberWidget.add( w );

      //check arrive max needed to show
      if( counterNeedToShow <= position ) {
        break;
      }
      position = position + 1;

    }

    //refresh
    setState(() {

    });
  }


  int _calculateCounterTabButtonNeedToShow(){
    int max = 7;
    if( DeviceTools.isPortrait( context) ){
      max = 3;
    }
    return max;
  }


  int _calculateStarterNeedoShow(){
    var defaultStarter = 1;
    //check there is not current page
    if( widget.currentPage == 0 ) {
      return defaultStarter;
    }


    //check more than range allowed
    //check max is more than totalBar pages
    // example: current page is 7, while totalBar is 14, i need to show only nearest nubmer of 5 button pages
    //           there is problem to show 7,8,9,10,11,12   because 11,12 not in range
    int getMax = widget.currentPage + _calculateCounterTabButtonNeedToShow();
    if( getMax > widget.pageTotal ) {
      return getMax - _calculateCounterTabButtonNeedToShow();
    }

    //check arrive maxPage
    // exmaple: current page is 995, while totalBar is 1001, while max page is 1000,
    //           i need to show 7 button pages
    //             there is problem to show 996,997,998,998,999,1000,1001,1002
    //             because 1001,1002   is over the max page allowed.
    //            at this time i need to show form 993 to 999
    if(  getMax  >= widget.maxPage!  ) {
      Log.i( "_calculateStarterNeedoShow() - getMax  >= maxPage  "   );
      return (widget.maxPage! ) - _calculateCounterTabButtonNeedToShow();
    }

    //now start from current page need
    return widget.currentPage;
  }

  //-----------------------------------------  fix screen change size in website

  void setupScreenChangeSizeDynamically(){
    ListenerChangeDeviceSize(context,
        orginal_width: DeviceTools.getWidth( context),
        orginal_height: DeviceTools.getHeight( context ),
        changeCallBack: (widthNew, heightNew) {

          //refresh
          setState(() {
            drawListNumberWidget();
          });
        }
    );
  }



}