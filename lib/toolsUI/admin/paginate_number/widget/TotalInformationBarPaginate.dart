...
extension TotalInformationBarPaginate on PaginateNumberState {
  Widget totalBarPaginate() {
    var rows = RowTemplate.wrapChildren([txt_total(), tf_pageGoTo(), bt_go()]);
  ...
  Widget txt_total() {
    var txt = "Total ${widget.pageTotal} pages Go To Page";
  ...
  void updateGoToPage(String newPage) {
    try {
       ...
       */
      Log.e("paginate - updateGoToPage() exc: $e");
    }
  }
  ...
  void goToClickFunction() {
    ...
    //more than maxPage
    Log.i("goToClickFunction() - maxPage: ${widget.maxPage}");
    if (pageGoTo >= widget.maxPage!) {
      ToolsToast.i(
          context, "Page more than Maximum Page is: ${widget.maxPage}");
      return;
    }
    ...
    if (pageGoTo > widget.pageTotal) {
      ToolsToast.i(
          context, "Page more than totalBar Page is: ${widget.pageTotal}");
      return;
    }
