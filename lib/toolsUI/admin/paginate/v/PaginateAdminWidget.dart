...
class PaginateAdminState extends State<PaginateAdminWidget> {
  ...
  int maxPage = 1000000; //pages: 999, means if there is 1000 page it will show to only page 999
  ...
  PaginateAdminState(this.callBack, {
    int? maxPage
  }){
  ...
  Widget build(BuildContext context) {
    var bars =  ResponsiveToLandscape.portraitVertical_to_landscapeHorizontal( context, children: [
      buttonBarPaginate(),
      totalBarPaginate()
    ]);
  ...
  void setPaginateByRecordTotal(int  totalRecord, int limitPerPage, int currentPage ) {
    ...
    pageTotal = ToolsNumber.roundToPlus1(total_page_double);
  ...
  void setMaxPage(int maxPage){
    this.maxPage = maxPage;
    Log.i( "setMaxPage() - maxPage: " + maxPage.toString()   );
  }
